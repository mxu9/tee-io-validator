...
/**
 * This function is to find empty key/iv slots for PCIE-IDE stream.
 *
...
bool pcie_ide_alloc_slot_ids(ide_common_test_port_context_t* port_context, uint8_t rp_stream_index, ide_key_set_t* k_set)
{
  ...
  // We use uint32_t as the bitmap to indicate the usage of key/iv slots.
  // In the future if the supported key/iv slots is more than 32, we will
  // revisit here.
  if(num_key_iv_slots > 32) {
    TEEIO_DEBUG((TEEIO_DEBUG_ERROR, "num_key_iv_slots exceeds 32 and the key_iv_slot_map shall be expanded.\n"));
    return false;
  }
  ...
  for(i = 0; i < num_stream_supported; i++) {
    ...
    key_iv_slot_map = (1 << keyset.pr) | key_iv_slot_map;
    key_iv_slot_map = (1 << keyset.npr) | key_iv_slot_map;
    key_iv_slot_map = (1 << keyset.cpl) | key_iv_slot_map;
  }

  TEEIO_DEBUG((TEEIO_DEBUG_INFO, "key/iv slot usage: key_iv_slot_map=%08x\n", key_iv_slot_map));
  for(i = 0; i < num_key_iv_slots / 3; i++) {
    uint8_t key_iv_slot = (key_iv_slot_map >> i*3) & 0x7;
    // it shall be either 000b or 111b
    if(key_iv_slot == 0b000) {
      ...
      found = true;
      break;
    } else if(key_iv_slot == 0b111) {
      // Skip to next 3 slots
    } else {
      TEEIO_ASSERT(false);
    }
  }
  ...
  if(found) {
    TEEIO_DEBUG((TEEIO_DEBUG_INFO, "Find empty key/iv slots: pr=%d, npr=%d, cpl=%d\n", i*3, i*3 + 1, i*3 + 2));
  } else {
    TEEIO_DEBUG((TEEIO_DEBUG_ERROR, "Failed to find empty key/iv slots.\n"));    
  }
