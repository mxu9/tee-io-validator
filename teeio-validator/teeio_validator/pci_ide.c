    ...
    const uint8_t key_set_select)
{
    ...
    TEEIO_DEBUG((TEEIO_DEBUG_INFO, "Before prime_host_ide_keys direction=%s ks=%s: %s=0x%08x\n",
    ...
    TEEIO_DEBUG((TEEIO_DEBUG_INFO, "After prime_host_ide_keys direction=%s ks=%s: %s=0x%08x\n",
      direct_names[direction],
      ks_names[key_set_select],
      ctrl_reg_names[direction],
      stream_txrx_control.raw));
    ...
    const uint8_t key_set_select)
{
    ...
    stream_txrx_control.raw = mmio_read_reg32(ctrl_reg_ptr);
    TEEIO_DEBUG((TEEIO_DEBUG_INFO, "After set_host_ide_key_set tx_control=0x%08x\n", stream_txrx_control.raw));
}
